...
fn policy<'a>() -> UpdateMetadataPolicy<'a> {
    UpdateMetadataPolicy {
        ...
        expected_version: Some("1.4.6"),
        expected_release_id: Some("v1.4.6"),
        expected_artifact_url_prefix: Some(
...
fn query<'a>() -> UpdateArtifactQuery<'a> {
    UpdateArtifactQuery {
        ...
        arch: "x86_64",
        format: "exe",
        file_name: Some("rustdesk-1.4.6-x86_64.exe"),
    }
}
...
fn rejects_artifact_file_name_path_components() {
    for file_name in ["nested/rustdesk.exe", r"nested\rustdesk.exe", ".."] {
        ...
        let mut path_query = query();
        path_query.file_name = None;

        let error = verify_with_query(&fixture, &path_query)
            .err()
...
fn verifies_metadata_generated_by_python_release_script_fixture() {
    ...
    let artifact =
        verify_update_metadata_with_keys(metadata, signature, &policy(), &query(), &[trusted_key])
