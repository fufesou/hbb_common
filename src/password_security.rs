...
// Keep enc_id downgrade-compatible with 1.4.6, which only understands zero-nonce storage.
pub(crate) fn encrypt_id_str_or_original_legacy_nonce(
    s: &str,
    version: &str,
    max_len: usize,
) -> String {
    ...
    if version == "00" {
        if let Ok(s) = encrypt_legacy_nonce(s.as_bytes()) {
            return version.to_owned() + &s;
        }
    }
    s.to_owned()
...
fn encrypt_legacy_nonce(v: &[u8]) -> Result<String, ()> {
    if !v.is_empty() {
        symmetric_crypt_legacy_nonce(v).map(|v| base64::encode(v, base64::Variant::Original))
    } else {
        Err(())
    }
}
...
fn symmetric_crypt_legacy_nonce(data: &[u8]) -> Result<Vec<u8>, ()> {
