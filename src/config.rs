...
use permanent_password::{
    ...
    is_current_permanent_password_encrypted_storage, looks_like_current_permanent_password_version,
    normalize_preset_password_storage, password_is_empty_or_not_hashed,
    permanent_password_storage_is_hashed, permanent_password_storage_matches_plain,
    preset_permanent_password_storage_matches_plain, DEFAULT_SALT_LEN, PASSWORD_ENC_VERSION,
    PERMANENT_PASSWORD_H1_LEN,
};
#[cfg(test)]
use permanent_password::{
    encode_permanent_password_storage_from_h1, is_permanent_password_hashed_storage,
...
impl Config {
    ...
    fn migrate_permanent_password_to_encrypted_hashed_storage(config: &mut Config) -> Result<bool> {
        ...
        if looks_like_current_permanent_password_version(&config.password) {
            ...
            if decrypted
                && decode_permanent_password_h1_from_hashed_storage(&decrypted_storage).is_some()
            {
                return Ok(false);
            }
            if !decrypted && is_current_permanent_password_encrypted_storage(&config.password) {
                return Ok(false);
            }
    ...
    pub fn matches_permanent_password_plain(input: &str) -> bool {
        ...
        if storage.is_empty() {
            let (preset_storage, preset_salt) = Self::get_preset_password_storage_and_salt();
            return preset_permanent_password_storage_matches_plain(
                &preset_storage,
                &preset_salt,
                input,
            );
        ...
        if decode_permanent_password_h1_from_storage(&storage).is_some() {
            return permanent_password_storage_matches_plain(&storage, &salt, input);
        }
    ...
    pub fn has_usable_preset_password() -> bool {
        let (preset_storage, preset_salt) = Self::get_preset_password_storage_and_salt();
...
mod tests {
    ...
    fn test_hbbs_00_hashed_preset_password_storage_matches_plain_with_salt() {
        ...
        with_config_and_hard_settings(Config::default(), hard_settings, || {
            ...
            assert!(Config::matches_permanent_password_plain("p@ssw0rd"));
            assert!(!Config::matches_permanent_password_plain("wrong"));
        });
    }
    ...
    fn test_legacy_plain_preset_password_with_00_hash_shape_without_salt_keeps_old_behavior() {
        ...
        with_config_and_hard_settings(config, hard_settings, || {
            ...
            assert!(Config::matches_permanent_password_plain(&storage));
            assert!(!Config::matches_permanent_password_plain("p@ssw0rd"));
        });
    }
    ...
    #[test]
    fn test_matches_plain_rejects_invalid_encrypted_local_storage() {
        ...
        let mut config = Config::default();
        config.password = locked_storage.clone();
        config.salt = "salt123".to_owned();

        with_config_and_hard_settings(config, HashMap::new(), || {
            assert!(!Config::matches_permanent_password_plain(&locked_storage));
        });
    }
    ...
    #[test]
    fn test_migrate_legacy_encrypted_plaintext_that_looks_like_00_hash() {
        let mut cfg = Config::default();
        cfg.salt = "salt123".to_owned();
        let plaintext_h1 = compute_permanent_password_h1("hash-looking-password", "other-salt");
        let plaintext = encode_permanent_password_storage_from_h1(&plaintext_h1);
        cfg.password = PASSWORD_ENC_VERSION.to_owned()
            + &base64::encode(
                crate::password_security::symmetric_crypt(plaintext.as_bytes(), true).unwrap(),
        ...
        let expected_h1 = compute_permanent_password_h1(&plaintext, "salt123");
        assert_eq!(stored_h1, expected_h1);
    }
    ...
    #[test]
    fn test_permanent_password_sync_rejects_plaintext_with_00_prefix() {
        let salt = "salt123";
        let mut cfg = Config::default();

        assert!(
            Config::apply_permanent_password_storage_for_sync(&mut cfg, "00secret", salt).is_err()
        );
        assert!(cfg.password.is_empty());
        assert!(cfg.salt.is_empty());
    }
    ...
    #[test]
    fn test_permanent_password_sync_rejects_encrypted_legacy_plaintext() {
        let salt = "salt123";
        let incoming =
            encrypt_str_or_original("legacy-secret", PASSWORD_ENC_VERSION, ENCRYPT_MAX_LEN);
        let mut cfg = Config::default();

        assert!(incoming.starts_with(PASSWORD_ENC_VERSION));
        assert!(
            Config::apply_permanent_password_storage_for_sync(&mut cfg, &incoming, salt).is_err()
        );
        assert!(cfg.password.is_empty());
        assert!(cfg.salt.is_empty());
    }
    ...
    #[test]
    fn test_permanent_password_sync_rejects_invalid_01_encrypted_storage() {
        let mut cfg = Config::default();
        let invalid_payload = vec![42u8; sodiumoxide::crypto::secretbox::MACBYTES + 1];
    ...
    #[test]
    fn test_permanent_password_sync_rejects_identical_invalid_01_storage() {
        let invalid_payload = vec![42u8; sodiumoxide::crypto::secretbox::MACBYTES + 1];
        let invalid_storage = PERMANENT_PASSWORD_ENC_VERSION.to_owned()
            + &base64::encode(invalid_payload, base64::Variant::Original);
        let mut cfg = Config::default();
    ...
    #[test]
    fn test_permanent_password_sync_rejects_plain_01_storage() {
        let mut cfg = Config::default();
    ...
    #[test]
    fn test_permanent_password_sync_rejects_decryptable_01_non_hash_storage() {
        ...
        assert!(
            Config::apply_permanent_password_storage_for_sync(&mut cfg, &storage, "salt123")
                .is_err()
        );
        assert!(cfg.password.is_empty());
        assert!(cfg.salt.is_empty());
    ...
    #[test]
    fn test_migrate_keeps_invalid_encrypted_permanent_password_storage() {
        let mut cfg = Config::default();
        let invalid_payload = vec![42u8; sodiumoxide::crypto::secretbox::MACBYTES + 1];
        cfg.password = PERMANENT_PASSWORD_ENC_VERSION.to_owned()
            + &base64::encode(invalid_payload, base64::Variant::Original);
